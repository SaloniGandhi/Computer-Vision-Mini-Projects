...
def computeHomography(f1, f2, matches, A_out=None):
    ...
    H = np.eye(3) #I
    # H=Vt[:,8].reshape(3,3)
    # print("inside alignment")
    # print(Vt.shape)
    # print(H)

    '''eigenvector of At A with smallest eigen value'''
    h=Vt[8]
    h=(h/Vt[8,8]).reshape(3,3)
    #print(h)
    #print(Vt[8])
    #BEGIN TODO 3
...
def alignPair(f1, f2, matches, m, nRANSAC, RANSACthresh):
    ...
    Input:
        ...
        nRANSAC -- number of RANSAC iterations #rounds
    ...
    for i in range(nRANSAC):
        ...
        if m==1:
            '''homograp'''
            match=minmatches
            H=computeHomography(f1,f2,match)

        inliers=getInliers(f1, f2, matches, H, RANSACthresh)
        if (len(inliers)>cnt_max_inliers):
            cnt_max_inliers=len(inliers)
            max_inliers=inliers
...
def leastSquaresFit(f1, f2, matches, m, inlier_indices):
    ...
    if m == eTranslate:
        ...
        for i in range(len(inlier_indices)):
            ...
            #over all inliers.
            #TODO-BLOCK-BEGIN
            f1_idx= matches[inlier_indices[i]].queryIdx
            f2_idx = matches[inlier_indices[i]].trainIdx
            '''u=u+xf2-xf1'''
            u+= f2[f2_idx].pt[0]-f1[f1_idx].pt[0]
            v+= f2[f2_idx].pt[1]-f1[f1_idx].pt[1]
        ...
        for i in inlier_indices:
            matches_on_inliers.append(matches[i])
        M=computeHomography(f1,f2,matches_on_inliers)
        #raise Exception("TODO in alignment.py not implemented")
        #TODO-BLOCK-END
        #END TODO
