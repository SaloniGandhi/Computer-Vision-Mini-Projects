...
def computeSphericalWarpMappings(dstShape, f, k1, k2):
    ...
    #print('inside warp')
    '''starts here'''
    '''xf, yf  theta and phi respectively'''
    x_spherical_coord = np.sin(xf)*np.cos(yf)
    y_spherical_coord = np.sin(yf)
    z_spherical_coord = np.cos(xf)*np.cos(yf)
    '''map onto z=1 plan ..divide by z'''
    xmap_norm=x_spherical_coord/z_spherical_coord
    ymap_norm=y_spherical_coord/z_spherical_coord
    '''add radial distortion with given k1 and k2'''
    r2=xmap_norm**2 + ymap_norm**2
    r4=r2**2
    xt=xmap_norm*(1+k1*r2+k2*r4)
    yt=ymap_norm*(1+k1*r2+k2*r4)
    '''end here'''
